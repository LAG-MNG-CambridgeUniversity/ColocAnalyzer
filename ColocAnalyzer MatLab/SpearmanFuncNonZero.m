...
%Here we assume pixels awhich are non-zero at two channels

%find all pixels with non-zero values in both channels
% 
NonZero1 = Channel1>0;
NonZero2 = Channel2>0;

%All non zero pixels in each channel
NonZero1Sum = sum(sum(Channel1));
NonZero2Sum = sum(sum(Channel2));

NonZero = NonZero1+NonZero2;
NonZeroAll = NonZero<2 == 0;

%Here are intensities of the pixel in each channel  
